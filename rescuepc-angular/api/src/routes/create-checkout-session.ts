...
createCheckoutSessionRouter.post('/', async (req, res) => {
  try {
    ...
    const { packageType, packageName, licenseCount = 1, successUrl, cancelUrl } = req.body;

    if (!packageType || !packageName) {
      return res.status(400).json({ error: 'Package type and name are required' });
    }

    const packageConfig = PAYMENT_PACKAGES[packageType as keyof typeof PAYMENT_PACKAGES];
    if (!packageConfig) {
